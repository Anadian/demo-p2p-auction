...
const uint = exports.uint = {
  preencode (state, n) {
	  console.log( "uint/preencode: state: %O len/n: %d", state, n );
  ...
  return {
    preencode (state, s) {
      const len = b4a.byteLength(s, encoding)
		console.log( "%s (%d) %d %O", s, s.length, len, state, encoding );
      uint.preencode(state, len)
      state.end += len
		console.log("string/preencode: %d", state.end);
    },
    encode (state, s) {
      const len = b4a.byteLength(s, encoding)
      uint.encode(state, len)
    ...
    fixed (n) {
      return {
        preencode (state) {
          state.end += n
			console.log("string/fixed/preencode: %d", state.end);
        },
        encode (state, s) {
...
const fixed = exports.fixed = function fixed (n) {
  return {
    preencode (state, s) {
      state.end += n
			console.log("fixed/preencode: %d", state.end);
    },
    encode (state, s) {
  ...
  return {
    preencode (state, m) {
      ...
      state.end += tmpBuf.byteLength
			console.log("fromCodec/preencode: %d", state.end);
    },
    encode (state, m) {
  ...
  return {
    preencode (state, m) {
      state.end += enc.encodingLength(m)
			console.log("fromAbstractEncoder/preencode: %d", state.end);
    },
    encode (state, m) {
...
exports.encode = function encode (enc, m) {
  ...
  enc.preencode(state, m)
  try{ 
	  /*if( Number.isNaN(state.end) === true ){
		  state.end = m.length;
	  }*/
	  state.buffer = b4a.allocUnsafe(state.end)
  } catch( error ){
	  console.log( "exports.encode: %o %s %o %s", enc, m, state, typeof(m) );
	  console.error( error );
  }
  enc.encode(state, m)
  return state.buffer
